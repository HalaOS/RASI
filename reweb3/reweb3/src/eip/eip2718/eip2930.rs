...
use crate::{
    errors::Result,
    primitives::{Bytes, Eip1559Signature, H256, U256},
    rlp::RlpEncoder,
};

use super::{keccak256, AccessList, LegacyTransactionRequest};
...
impl Eip2930TransactionRequest {
    ...
    pub fn rlp(&self) -> Result<Bytes> {
        ...
        let chain_id = self.tx.chain_id.unwrap_or(U256::from(1u32));
...
mod tests {
    ...
    fn test_rlp_failed() {
        ...
        assert_eq!(tx.tx.nonce, Some(U256::from(3u32)));
        assert_eq!(tx.tx.gas_price, Some(U256::from(1u32)));
        assert_eq!(tx.tx.gas, Some(U256::from(25000u32)));
        assert_eq!(tx.tx.value, Some(U256::from(10u32)));
        assert_eq!(tx.tx.data, Some(Bytes::from(vec![0x55u8, 0x44])));
