...
use crate::primitives::{Address, Bytes, Hex, H256, U256};
use regex::Regex;
...
impl TupleEncoder {
    ...
    fn finalize(self, type_hash: [u8; 32], definition: TypeDefinition) -> [u8; 32] {
        ...
        for field in definition {
            ...
            log::debug!(
                "hash update element {} with index {}: {}",
                field.name,
                index,
                Hex(&self.fields[index])
            );
...
impl<'a, 'b> Serializer for &'a mut EIP712StructHasher<'b> {
    ...
    fn serialize_str(self, v: &str) -> Result<Self::Ok, Self::Error> {
        if let Some(type_name) = self.element_type_name()? {
            log::debug!("serialize_str {}, type_name {}", v, type_name);
            match type_name.as_str() {
                "bytes" | "bytesN" => {
                    let bytes = Bytes::from_str(v).map_err(ser::Error::custom)?;
                ...
                "uint256" => {
                    let value = if v.starts_with("0x") {
                        U256::from_str_radix(&v[2..], 16).map_err(ser::Error::custom)?
                    } else {
                        U256::from_str_radix(v, 10).map_err(ser::Error::custom)?
                    };
