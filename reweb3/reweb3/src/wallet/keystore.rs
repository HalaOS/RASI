...
pub enum KeyStoreError {
    ...
    #[error(transparent)]
    EllipticError(#[from] elliptic_curve::Error),
}
...
pub trait KeyStoreEncrypt {
    ...
    fn encrypt_with<R, B, P>(rng: &mut R, pk: B, password: P) -> Result<KeyStore, KeyStoreError>
    where
        R: Rng + CryptoRng,
        P: AsRef<[u8]>,
        B: AsRef<[u8]>,
    {
        ...
        let sec_key = SecretKey::from_slice(&ciphertext)?;
        ...
        S: AsRef<[u8]>,
    {
        ...
        let key = match keystore.crypto.kdfparams {
            ...
            KdfparamsType::Scrypt {
                ...
                let scrypt_params = ScryptParams::new(log_n, r, p, dklen as usize)
                    .map_err(|err| KeyStoreError::Scrypt(err.to_string()))?;
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        let keystore = KeyStore::encrypt(
            Bytes::from_str("0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80")
                .unwrap(),
    ...
    fn test_decrypt_pbkdf2() {
        let keystore: KeyStore = include_str!("keys/pbkdf2.json")
            .parse()
    ...
    fn test_decrypt_scrypt() {
        ...
        let keystore: KeyStore = include_str!("keys/scrypt.json")
            .parse()
            .expect("Load keystore from file");
    ...
    fn test_decrypt_ethersjs_keystore() {
        let keystore: KeyStore = include_str!("keys/ethersjs.json")
            .parse()
            .expect("Load keystore from file");

        let expect_address = keystore.address.clone();

        let pk = SecretKey::from_slice(
            &keystore
                .decrypt_into("test")
                .expect("Decrypt keystore with password"),
        )
