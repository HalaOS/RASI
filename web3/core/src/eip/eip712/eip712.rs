    ...
    M: Serialize,
{
    ...
    pub fn encode(&self) -> Result<[u8; 66]> {
        ...
        buff[2..34].copy_from_slice(domain.as_ref());
        buff[34..66].copy_from_slice(&message.as_ref());
...
pub fn eip712_into_request<S: Serialize>(domain: EIP712Domain, value: S) -> Result<TypedData<S>> {
    // Get primary type.
